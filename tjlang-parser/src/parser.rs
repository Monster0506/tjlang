...
impl PestParser {
    ...
    fn add_parse_error(&mut self, code: tjlang_diagnostics::ErrorCode, message: String, span: pest::Span) {
        ...
        let source_span = self.create_diagnostic_span(span, file_id);
    ...
    fn analyze_error_context(&self, source: &str, pos: usize) -> ErrorContext {
        ...
        if before.ends_with("def ") {
            ...
            context.note = Some("while loop syntax: while condition { body }".to_string());
        ...
        match char_at_pos {
            ...
            _ => {
                ...
                let message = if context.before.trim().is_empty() {
                    "unexpected token at start of input".to_string()
                } else if context.after.trim().is_empty() {
                    "unexpected token at end of input".to_string()
                } else {
        ...
        match pair.as_rule() {
            ...
            Rule::unary => {
                ...
                if let Some(first) = inner.next() {
                    ...
                    match first.as_rule() {
                        Rule::range_expr => {
                            // This is a range expression without a unary operator
                            self.parse_range_expr(first)
                        }
                        Rule::postfix_expr => {
                            // This is a postfix expression without a unary operator
                            self.parse_postfix_expr(first)
                        }
                        _ => {
    ...
    fn parse_range_expr(&mut self, pair: Pair<Rule>) -> Result<Expression, Box<dyn std::error::Error>> {
        ...
        // Skip range_op
        let op_pair = inner.next().ok_or("Missing range operator")?;
        if op_pair.as_rule() != Rule::range_op {
            return Err(format!("Expected range operator, got {:?}", op_pair.as_rule()).into());
        }

        // Parse end value (must exist for our current semantics) - should be postfix_expr_no_range
        let end_pair = inner.next().ok_or("Missing end value")?;
        let end = match end_pair.as_rule() {
            Rule::postfix_expr_no_range => {
                // Parse postfix_expr_no_range which uses primary_no_range
    ...
    fn parse_interface_decl(&mut self, pair: Pair<Rule>) -> Result<InterfaceDecl, Box<dyn std::error::Error>> {
        ...
        // debug_println!("DEBUG: Interface declaration inner pairs:");
        // for (i, p) in inner_pairs.iter().enumerate() {
        //     debug_println!("  {}: Rule={:?}, Content='{}'", i, p.as_rule(), p.as_str());
        // }
        ...
        // debug_println!("DEBUG: Parsed interface name: '{}'", name);
