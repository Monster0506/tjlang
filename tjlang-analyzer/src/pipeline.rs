...
impl AnalysisPipeline {
    ...
    pub fn analyze_phase(&self, phase: AnalysisPhase, source: &str, file_id: codespan::FileId) -> AnalysisResult {
        match phase {
            AnalysisPhase::PreAST => {
                let context = AnalysisContext::new(source.to_string(), file_id, self.config.clone());
    ...
    pub fn analyze_category(&self, category: RuleCategory, source: &str, file_id: codespan::FileId) -> AnalysisResult {
        let start_time = Instant::now();
        let mut all_diagnostics = DiagnosticCollection::new();
        let mut rule_results = Vec::new();
    ...
    pub fn get_rules_by_priority(&self, min_priority: u32, max_priority: u32) -> Vec<&dyn AnalysisRule> {
        self.rules.iter()
...
impl AnalysisResult {
    ...
    pub fn get_summary(&self) -> AnalysisSummary {
        AnalysisSummary {
            ...
            error_count: self.diagnostics.get_diagnostics_by_severity(Severity::Error).len(),
            warning_count: self.diagnostics.get_diagnostics_by_severity(Severity::Warning).len(),
            info_count: self.diagnostics.get_diagnostics_by_severity(Severity::Note).len(),
