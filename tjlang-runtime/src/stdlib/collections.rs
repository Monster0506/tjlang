...
impl COLLECTIONS {
    ...
    pub fn array_with_capacity<T>(capacity: usize) -> Vec<T> {
        Vec::with_capacity(capacity)
    }
    pub fn array_from_slice<T: Clone>(slice: &[T]) -> Vec<T> {
        slice.to_vec()
    }

    pub fn array_push<T>(vec: &mut Vec<T>, item: T) {
        vec.push(item);
    }
    pub fn array_pop<T>(vec: &mut Vec<T>) -> Option<T> {
        vec.pop()
    }
    ...
    pub fn array_len<T>(vec: &Vec<T>) -> usize {
        vec.len()
    }
    pub fn array_capacity<T>(vec: &Vec<T>) -> usize {
        vec.capacity()
    }
    pub fn array_is_empty<T>(vec: &Vec<T>) -> bool {
        vec.is_empty()
    }

    pub fn array_get<T>(vec: &Vec<T>, index: usize) -> Option<&T> {
        vec.get(index)
    }
    pub fn array_get_mut<T>(vec: &mut Vec<T>, index: usize) -> Option<&mut T> {
        vec.get_mut(index)
    }
    ...
    pub fn array_append<T>(vec: &mut Vec<T>, other: &mut Vec<T>) {
        vec.append(other);
    }
    pub fn array_extend<T: Clone>(vec: &mut Vec<T>, other: &[T]) {
        vec.extend_from_slice(other);
    }

    pub fn array_reverse<T>(vec: &mut Vec<T>) {
        vec.reverse();
    }
    pub fn array_sort<T: Ord>(vec: &mut Vec<T>) {
        vec.sort();
    }
    ...
    pub fn map_len<K, V>(map: &HashMap<K, V>) -> usize {
        map.len()
    }
    ...
    pub fn map_keys<K, V>(map: &HashMap<K, V>) -> Vec<&K> {
        map.keys().collect()
    }
    pub fn map_values<K, V>(map: &HashMap<K, V>) -> Vec<&V> {
        map.values().collect()
    }
    pub fn map_entries<K, V>(map: &HashMap<K, V>) -> Vec<(&K, &V)> {
        map.iter().collect()
    }
    ...
    pub fn set_from_vec<T: std::hash::Hash + Eq>(vec: Vec<T>) -> HashSet<T> {
    ...
    pub fn set_len<T>(set: &HashSet<T>) -> usize {
        set.len()
    }
    ...
    pub fn set_union<T: std::hash::Hash + Eq + Clone>(
        set1: &HashSet<T>,
        set2: &HashSet<T>,
    ) -> HashSet<T> {
    ...
    pub fn set_intersection<T: std::hash::Hash + Eq + Clone>(
        set1: &HashSet<T>,
        set2: &HashSet<T>,
    ) -> HashSet<T> {
    ...
    pub fn set_difference<T: std::hash::Hash + Eq + Clone>(
        set1: &HashSet<T>,
        set2: &HashSet<T>,
    ) -> HashSet<T> {
    ...
    pub fn set_symmetric_difference<T: std::hash::Hash + Eq + Clone>(
        set1: &HashSet<T>,
        set2: &HashSet<T>,
    ) -> HashSet<T> {
    ...
    pub fn queue_with_capacity<T>(capacity: usize) -> VecDeque<T> {
        VecDeque::with_capacity(capacity)
    }

    pub fn queue_push_back<T>(queue: &mut VecDeque<T>, item: T) {
        queue.push_back(item);
    }
    pub fn queue_push_front<T>(queue: &mut VecDeque<T>, item: T) {
        queue.push_front(item);
    }
    pub fn queue_pop_back<T>(queue: &mut VecDeque<T>) -> Option<T> {
        queue.pop_back()
    }
    pub fn queue_pop_front<T>(queue: &mut VecDeque<T>) -> Option<T> {
        queue.pop_front()
    }

    pub fn queue_len<T>(queue: &VecDeque<T>) -> usize {
        queue.len()
    }
    pub fn queue_is_empty<T>(queue: &VecDeque<T>) -> bool {
        queue.is_empty()
    }

    // Priority queue operations
    pub fn priority_queue_new<T: Ord>() -> BinaryHeap<T> {
        BinaryHeap::new()
    }
    pub fn priority_queue_with_capacity<T: Ord>(capacity: usize) -> BinaryHeap<T> {
        BinaryHeap::with_capacity(capacity)
    }

    pub fn priority_queue_push<T: Ord>(queue: &mut BinaryHeap<T>, item: T) {
        queue.push(item);
    }
    pub fn priority_queue_pop<T: Ord>(queue: &mut BinaryHeap<T>) -> Option<T> {
        queue.pop()
    }
    pub fn priority_queue_peek<T: Ord>(queue: &BinaryHeap<T>) -> Option<&T> {
        queue.peek()
    }

    pub fn priority_queue_len<T: Ord>(queue: &BinaryHeap<T>) -> usize {
        queue.len()
    }
    pub fn priority_queue_is_empty<T: Ord>(queue: &BinaryHeap<T>) -> bool {
        queue.is_empty()
    }
    ...
    pub fn iterator_count<T>(iter: impl Iterator<Item = T>) -> usize {
        iter.count()
    }
    ...
    pub fn iterator_min<T: Ord>(iter: impl Iterator<Item = T>) -> Option<T> {
        iter.min()
    }
    pub fn iterator_max<T: Ord>(iter: impl Iterator<Item = T>) -> Option<T> {
        iter.max()
    }

    pub fn iterator_zip<T, U>(
        iter1: impl Iterator<Item = T>,
        iter2: impl Iterator<Item = U>,
    ) -> impl Iterator<Item = (T, U)> {
        iter1.zip(iter2)
    }

    pub fn iterator_chain<T>(
        iter1: impl Iterator<Item = T>,
    ...
    pub fn iterator_enumerate<T>(
        iter: impl Iterator<Item = T>,
    ) -> impl Iterator<Item = (usize, T)> {
        iter.enumerate()
    }

    pub fn iterator_rev<T>(
    ...
    pub fn rotate_left<T>(vec: &mut Vec<T>, mid: usize) {
        vec.rotate_left(mid);
    }
    pub fn rotate_right<T>(vec: &mut Vec<T>, k: usize) {
        vec.rotate_right(k);
    }
    ...
    pub fn counter_from_iter<T: std::hash::Hash + Eq + Clone, I: Iterator<Item = T>>(
        iter: I,
    ) -> Counter<T> {
        Counter::from_iter(iter)
    }

    pub fn counter_most_common<T: std::hash::Hash + Eq + Clone>(
        counter: &Counter<T>,
        n: Option<usize>,
    ) -> Vec<(T, usize)> {
    ...
    pub fn counter_subtract<T: std::hash::Hash + Eq + Clone>(
    ...
    pub fn counter_update<T: std::hash::Hash + Eq + Clone, I: Iterator<Item = T>>(
        counter: &mut Counter<T>,
        iter: I,
    ) {
        counter.update(iter);
    ...
    pub fn default_dict_new<F: Fn() -> V, K: std::hash::Hash + Eq + Clone, V: Clone>(
        default_factory: F,
    ) -> DefaultDict<K, V, F> {
        DefaultDict::new(default_factory)
    }

    pub fn default_dict_get<F: Fn() -> V, K: std::hash::Hash + Eq + Clone, V: Clone>(
        dict: &mut DefaultDict<K, V, F>,
        key: K,
    ) -> V {
        dict.get(key)
    }

    pub fn default_dict_set<F: Fn() -> V, K: std::hash::Hash + Eq + Clone, V: Clone>(
        dict: &mut DefaultDict<K, V, F>,
        key: K,
        value: V,
    ) {
    ...
    pub fn chain_map_add<K: std::hash::Hash + Eq + Clone, V: Clone>(
        chain: &mut ChainMap<K, V>,
        map: HashMap<K, V>,
    ) {
        chain.add(map);
    }

    pub fn chain_map_get<'a, K: std::hash::Hash + Eq + Clone, V: Clone>(
        chain: &'a ChainMap<K, V>,
        key: &'a K,
    ) -> Option<&'a V> {
        chain.get(key)
    }
    ...
    pub fn named_tuple_set<T: Clone>(
        nt: &mut NamedTuple<T>,
        field: &str,
        value: T,
    ) -> Result<(), String> {
    ...
    pub fn ordered_dict_move_to_end<K: std::hash::Hash + Eq + Clone, V: Clone>(
        dict: &mut OrderedDict<K, V>,
        key: &K,
    ) {
        dict.move_to_end(key);
    }

    pub fn ordered_dict_popitem<K: std::hash::Hash + Eq + Clone, V: Clone>(
        dict: &mut OrderedDict<K, V>,
        last: bool,
    ) -> Option<(K, V)> {
        dict.popitem(last)
