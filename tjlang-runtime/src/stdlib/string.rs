...
impl STRING {
    ...
    pub fn byte_count(s: &str) -> usize { s.len() }
    ...
    // String searching
    pub fn contains(s: &str, pattern: &str) -> bool { s.contains(pattern) }
    pub fn starts_with(s: &str, prefix: &str) -> bool { s.starts_with(prefix) }
    pub fn ends_with(s: &str, suffix: &str) -> bool { s.ends_with(suffix) }
    
    pub fn find(s: &str, pattern: &str) -> Option<usize> { s.find(pattern) }
    pub fn rfind(s: &str, pattern: &str) -> Option<usize> { s.rfind(pattern) }
    ...
    pub fn pad_left(s: &str, width: usize, fill_char: char) -> String {
        if s.len() >= width { s.to_string() } else {
    ...
    pub fn is_whitespace(s: &str) -> bool { s.chars().all(|c| c.is_whitespace()) }
    pub fn is_alpha(s: &str) -> bool { s.chars().all(|c| c.is_alphabetic()) }
    pub fn is_numeric(s: &str) -> bool { s.chars().all(|c| c.is_numeric()) }
    pub fn is_alphanumeric(s: &str) -> bool { s.chars().all(|c| c.is_alphanumeric()) }
    pub fn is_ascii(s: &str) -> bool { s.is_ascii() }
    pub fn is_digit(s: &str) -> bool { s.chars().all(|c| c.is_ascii_digit()) }
    pub fn is_hex(s: &str) -> bool { s.chars().all(|c| c.is_ascii_hexdigit()) }
    
    // String conversion
    pub fn to_ascii(s: &str) -> String { s.to_ascii_lowercase() }
    pub fn to_unicode(s: &str) -> Vec<u32> { s.chars().map(|c| c as u32).collect() }
    pub fn from_unicode(codes: &[u32]) -> String {
        codes.iter().filter_map(|&code| char::from_u32(code)).collect()
    }
    ...
    pub fn to_camel_case(s: &str) -> String {
        ...
        let other_words: String = words[1..].iter()
    ...
    // String statistics
    pub fn word_count(s: &str) -> usize { s.split_whitespace().count() }
    pub fn line_count(s: &str) -> usize { s.lines().count() }
    ...
    // String cleaning
    pub fn remove_whitespace(s: &str) -> String { s.chars().filter(|c| !c.is_whitespace()).collect() }
    pub fn remove_punctuation(s: &str) -> String { s.chars().filter(|c| !c.is_ascii_punctuation()).collect() }
    pub fn remove_digits(s: &str) -> String { s.chars().filter(|c| !c.is_ascii_digit()).collect() }
    pub fn remove_alpha(s: &str) -> String { s.chars().filter(|c| !c.is_alphabetic()).collect() }
    ...
    pub fn normalize_case(s: &str) -> String { s.to_lowercase() }
    pub fn normalize_unicode(s: &str) -> String { s.to_string() } // TODO: Implement Unicode normalization
    
    // String comparison
    pub fn equals_ignore_case(s1: &str, s2: &str) -> bool { s1.eq_ignore_ascii_case(s2) }
    pub fn compare(s1: &str, s2: &str) -> i32 { s1.cmp(s2) as i32 }
