...
impl FILE {
    ...
    fn find_files_recursive_helper(path: &str, pattern: &str, matches: &mut Vec<String>) -> Result<(), String> {
        let entries = Self::list_dir(path)?;
        for entry in entries {
    ...
    pub fn extension(path: &str) -> Option<String> {
        Path::new(path).extension().map(|ext| ext.to_string_lossy().to_string())
    }
    
    /// Get file stem (name without extension)
    pub fn stem(path: &str) -> Option<String> {
        Path::new(path).file_stem().map(|stem| stem.to_string_lossy().to_string())
    }
    ...
    pub fn parent(path: &str) -> Option<String> {
        Path::new(path).parent().map(|parent| parent.to_string_lossy().to_string())
    }
    ...
    pub fn current_dir() -> Result<String, String> {
        std::env::current_dir().map_err(|e| e.to_string()).map(|path| path.to_string_lossy().to_string())
    }
    ...
    pub fn watch_file(path: &str, callback: Box<dyn Fn() -> Result<(), String>>) -> Result<FileWatcher, String> {
