...
impl TIME {
    ...
    pub fn parse_date(date_str: &str, format: &str) -> Result<u64, String> {
        let date =
            chrono::NaiveDate::parse_from_str(date_str, format).map_err(|e| e.to_string())?;
    ...
    pub fn from_components(
        ...
        let time = chrono::NaiveTime::from_hms_opt(hour, minute, second).ok_or("Invalid time")?;
    ...
    pub fn get_age(birth_timestamp: u64) -> u32 {
        let now = Self::now();
        let birth_date =
            chrono::DateTime::from_timestamp(birth_timestamp as i64, 0).unwrap_or_default();
